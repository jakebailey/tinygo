...
TEST_PACKAGES_CALLERS_HOST := $(filter io log,$(TEST_PACKAGES_HOST))
...
tinygo-test:
	...
	$(TINYGO) test $(TEST_ADDITIONAL_FLAGS) $(TEST_SKIP_FLAG) $(filter-out encoding/xml $(TEST_PACKAGES_SHORT) $(TEST_PACKAGES_CALLERS_HOST),$(TEST_PACKAGES_HOST) $(TEST_PACKAGES_SLOW))
ifneq ($(TEST_PACKAGES_SHORT_HOST),)
	$(TINYGO) test $(TEST_ADDITIONAL_FLAGS) $(TEST_SKIP_FLAG) -short $(TEST_PACKAGES_SHORT_HOST)
endif
ifneq ($(TEST_PACKAGES_CALLERS_HOST),)
	$(TINYGO) test $(TEST_ADDITIONAL_FLAGS) -skip='^(TestCallDepth|TestMultiWriter_WriteStringSingleAlloc|TestMultiReaderFreesExhaustedReaders)$$' $(TEST_PACKAGES_CALLERS_HOST)
endif
