...
type libraryCacheInput struct {
	...
	SourceDir        string
	InputDir         string
	HeaderInputs     map[string]string
...
func makeLibraryCacheKeys(config *compileopts.Config, libraries configuredLibrarySet) (map[string]string, error) {
...
func (l *Library) cacheInput(config *compileopts.Config) (*libraryCacheInput, error) {
	...
	input := libraryCacheInput{
		...
		SourceDir:        sourceDir,
		InputDir:         inputDir,
		HeaderInputs:     make(map[string]string),
	...
	headerInputs, err := hashLibraryHeaderInputs(inputDir)
	if err != nil {
		return nil, err
	}
...
func (l *Library) hashGeneratedHeaders(target string) (map[string]string, error) {
	...
	return hashLibraryHeaderInputs(dir)
}

func hashLibraryHeaderInputs(root string) (map[string]string, error) {
...
func libraryCompilerIdentity() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to identify clang: %w", err)
	}
	return name + "\n" + string(out), nil
}
